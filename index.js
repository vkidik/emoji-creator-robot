...
bot.on('photo', async (ctx) => {
  try {
    ...
    if(ctx.message.caption){
      let line = ctx.message.caption;
      if(line.includes('-rbg')){
        args.chromo = line.split('-rbg ')[1].split(' ')[0];
      }
    }
...
bot.on('video', async (ctx) => {
  try {
    console.log('Получено видео от пользователя', ctx.from.id);
    
    const video = ctx.message.video;
    

    // Проверка по file_id: если видео уже обрабатывалось ранее, не добавляем в очередь
    if (processedFileIds.has(video.file_id)) {
      console.log(`Фото с file_id ${video.file_id} уже обработано, повторная обработка пропущена.`);
      return;
    }
    processedFileIds.add(video.file_id);
    ...
    if(ctx.message.caption){
      ...
      if(line.includes('-sim')){
        args.similarity = line.split('-sim ')[1].split(' ')[0];
      }
    }
